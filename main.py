...
# send event message request
socket.send(buf)
response = socket.recv(8)
(version, messageType, length) = struct.unpack('>HHL', response)
...
# f = open(r'C:\Users\User\Desktop\encoreApp\Estreamer\data.txt', "rb")
# data = f.read()
# f.close()
offset = 0
serviceId = 0
...
while offset < len(data):
    weeBuffer = data[offset:offset + 8]
...
buf1 = struct.pack('>HHLLLLLHHHH', *[1, 2, 24, 6667, 0, flags, 0, 16, 71, 4, 21])
sock.send(buf)
